...
function okOrThrow(error) { if (error) throw error; }

export async function listarLecturas(grado) {
  const { data, error } = await supabase.rpc('admin_listar_lecturas', { p_grado: String(grado) });
  okOrThrow(error); return data || [];
}
export async function listarDesafios(grado) {
  const { data, error } = await supabase.rpc('admin_listar_desafios', { p_grado: String(grado) });
  okOrThrow(error); return data || [];
}
export async function listarJuegos(grado, habilidad = null) {
  const { data, error } = await supabase.rpc('admin_listar_juegos', { p_grado: String(grado), p_habilidad: habilidad });
  okOrThrow(error); return data || [];
}
export async function crearCiclo({ grado, nombre, descripcion, lecturaId, desafioId, juegosIds }) {
  const { data, error } = await supabase.rpc('admin_ciclo_crear', {
    p_grado: String(grado), p_nombre: nombre, p_descripcion: descripcion || '',
    p_lectura_id: lecturaId, p_desafio_id: desafioId, p_juegos: juegosIds
  });
  okOrThrow(error); return data;
}
